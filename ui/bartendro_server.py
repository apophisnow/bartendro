...
RPI_CPU_IDS_FOR_TTYAMA0 = ["0002", "0003", "0004", "0005",
                           "0006", "0007", "0008", "0009",
                           "000d", "000e", "000f", "0010",
                           "0013", "0011", "0012", "a01041",
                           "a21041", "900092"]
...
if os.path.exists("version.txt"):
    ...
    if version:
        version = "git commit " + str(version[:10])
    else:
        version = "[unknown]"
...
parser.add_argument(
    "-d",
    "--debug",
    help="Turn on debugging mode to see stack traces in the error log",
    default=True,
    action='store_true'
)
parser.add_argument(
    "-t",
    "--host",
    help="Which interfaces to listen on. Default: 127.0.0.1",
    default="127.0.0.1",
    type=str
)
parser.add_argument(
...
parser.add_argument(
    "-s",
    "--software-only",
    help="Run only the server software, without hardware interaction.",
    default=False,
    action='store_true'
)
...
handler = logging.handlers.RotatingFileHandler(
    os.path.join("logs", "bartendro.log"),
    maxBytes=LOG_SIZE,
